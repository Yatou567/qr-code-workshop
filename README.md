...
- **生成历史**：网页和插件各自在本机保留最近 100 条记录，同内容去重，支持搜索、查看、单条删除和清空。
- **选择网页元素**：插件点击「选取元素」，在网页上点选文字、链接或图片；浮层直接显示内容和二维码，可编辑内容后更新。结果自动进入插件历史。
- **复制二维码图片**：网页、插件弹窗和元素浮层均可复制 PNG 图片，直接粘贴到支持图片的聊天或文档中。

插件升级：解压最新 ZIP 后在 `chrome://extensions` 重新加载对应文件夹；新增 `storage` 和 `clipboardWrite` 权限用于本地历史与复制图片，浏览器如提示请确认。不要卸载旧插件后再安装，否则原插件数据可能丢失。
...
- 插件请求 `activeTab`、`scripting`、`storage` 和 `clipboardWrite`，用于用户打开插件时临时读取当前页面。
- 只保存成功生成的二维码内容、来源类型和时间到本地历史，不使用云同步；可以删除。
...
- 不解析没有 `href`、只由 JavaScript 控制跳转的按钮；元素选取可读取这类元素显示的文字。
...
插件的当前网页、点击保留的内容、稳定悬停约 0.7 秒的链接及停止输入约 0.8 秒后的有效内容会记入历史；快速掠过链接或尚未完成的输入不会逐字记录。网页版在点击生成成功后保存，初始示例不自动入历史。历史保存原始内容，不是图片缩略图，点击后可重新生成、复制或下载。

网页版与插件历史互不共享；不同浏览器、不同网页版来源也不互通。本地 `file://` 的存储行为取决于浏览器，移动 HTML 文件可能导致原历史不可见。浏览器禁止本地存储时会提示，当前页面仍能生成二维码。

元素选择支持当前主文档及开放 Shadow DOM，不穿透 iframe，也不会读取表单输入值或编辑区域。图片模式提取图片地址，不做 OCR。按 Esc 或关闭按钮退出选择。超长内容可在浮层编辑后再生成。

复制图片需要浏览器支持 Clipboard API 并允许写入；HTTPS 网页与 Chrome 插件优先。受限页面、HTTP 页面或部分本地文件环境可能无法复制，此时请下载 PNG 或在插件弹窗复制。
